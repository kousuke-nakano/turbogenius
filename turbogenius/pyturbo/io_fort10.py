...
from utils.utility import pygrep_lineno, pysed_replace, pygetline, pysed_replace_lines
from utils.utility import return_orb_type_chr, return_contraction_flag, return_num_twobody_and_flag_onebody
from utils.utility import return_element_symbol
from structure import Structure, Cell
from basis_set import Det_Basis_sets, Jas_Basis_sets
...
class IO_fort10:
    ...
    f10detbasis_sym_start_keyword = "Eq. par. in the atomic Det par.in the chosen basis *$"
    f10detbasis_sym_end_keyword = "Eq. par. in the atomic 3-body  par. in the chosen basis *$"
    f10jasbasis_sym_start_keyword = "Eq. par. in the atomic 3-body  par. in the chosen basis *$"
    f10jasbasis_sym_end_keyword = "New parameters *$"

    def __init__(self, fort10):

        self.fort10 = fort10
        self.f10header = F10header(fort10=self.fort10)
        logger.debug("header")
        self.f10structure = F10structure(fort10=self.fort10, Nel=self.f10header.nel, Ion=self.f10header.natom,
                                         start_keyword=self.f10structure_start_keyword, end_keyword=self.f10structure_end_keyword)
        logger.debug("structure")
        self.f10forceconstraint = F10forceconstraint(fort10=self.fort10, ieskinr=self.f10header.ieskinr,
                                                     start_keyword=self.f10forceconstraint_start_keyword, end_keyword=self.f10forceconstraint_end_keyword)
        logger.debug("f10forceconstraint")
        self.f10jastwobody = F10jastwobody(fort10=self.fort10, jas_2body=self.f10header.jas_2body,
                                           start_keyword=self.f10jastwobody_start_keyword, end_keyword=self.f10jastwobody_end_keyword)
        logger.debug("f10jastwobody")
        self.f10detbasissets = F10detbasissets(fort10=self.fort10, shell_det=self.f10header.shell_det, complex_flag=self.f10header.complex_flag,
                                               start_keyword=self.f10detbasis_start_keyword, end_keyword=self.f10detbasis_end_keyword)
        ...
        self.f10detmat_sym = F10matsymmetry(fort10=self.fort10, num_component=self.f10header.iessw,
                                           start_keyword=self.f10detmatsym_start_keyword, end_keyword=self.f10detmatsym_end_keyword)
        logger.debug("f10detmat_sym")
        self.f10jasmatrix = F10jasmat(fort10=self.fort10, jasmat=self.f10header.jas_mat_nonzero,
                                      start_keyword=self.f10jasmat_start_keyword, end_keyword=self.f10jasmat_end_keyword)
        logger.debug("f10jasmatrix")
        self.f10jasmat_sym = F10matsymmetry(fort10=self.fort10, num_component=self.f10header.iesfree,
                                           start_keyword=self.f10jasmatsym_start_keyword, end_keyword=self.f10jasmatsym_end_keyword)
        logger.debug("f10jasmat_sym")
        self.f10detbasis_sym = F10basissymmetry(fort10=self.fort10, num_component=self.f10header.eq_det_atomic_par,
                                           start_keyword=self.f10detbasis_sym_start_keyword, end_keyword=self.f10detbasis_sym_end_keyword)
        logger.debug("f10detbasis_sym")
        self.f10jasbasis_sym = F10basissymmetry(fort10=self.fort10, num_component=self.f10header.eq_3_body_atomic_par,
                                           start_keyword=self.f10jasbasis_sym_start_keyword, end_keyword=self.f10jasbasis_sym_end_keyword)
...
class F10header():
    ...
    def read(self):
        if not self.read_flag:
            with open(self.fort10, "r") as f:
                ...
                self.__Nelup = Value(value=int(line[0]), lineno=lineno, index=0, file=self.fort10)
                self.__Nel = Value(value=int(line[1]), lineno=lineno, index=1, file=self.fort10)
                self.__Ion = Value(value=int(line[2]), lineno=lineno, index=2, file=self.fort10)
                lineno+=2
                line = pygetline(filename=self.fort10, lineno=lineno).split()
                self.__Shell_Det=Value(value=int(line[0]), lineno=lineno, index=0, file=self.fort10)
                self.__Shell_Jas=Value(value=int(line[1]), lineno=lineno, index=1, file=self.fort10)
                lineno+=2
                line = pygetline(filename=self.fort10, lineno=lineno).split()
                self.__Jas_2body=Value(value=int(line[0]), lineno=lineno, index=0, file=self.fort10)
                self.__Det=Value(value=int(line[1]), lineno=lineno, index=1, file=self.fort10)
                self.__three_body_atomic_par=Value(value=int(line[2]), lineno=lineno, index=2, file=self.fort10)
                lineno+=2
                line = pygetline(filename=self.fort10, lineno=lineno).split()
                self.__Det_mat_nonzero=Value(value=int(line[0]), lineno=lineno, index=0, file=self.fort10)
                self.__Jas_mat_nonzero=Value(value=int(line[1]), lineno=lineno, index=1, file=self.fort10)
                lineno+=2
                line = pygetline(filename=self.fort10, lineno=lineno).split()
                self.__Eq_Det_atomic_par=Value(value=int(line[0]), lineno=lineno, index=0, file=self.fort10)
                self.__Eq_3_body_atomic_par=Value(value=int(line[1]), lineno=lineno, index=1, file=self.fort10)
                lineno+=2
                line = pygetline(filename=self.fort10, lineno=lineno).split()
                self.__iesfree=Value(value=int(line[0]), lineno=lineno, index=0, file=self.fort10)
                self.__iessw=Value(value=int(line[1]), lineno=lineno, index=1, file=self.fort10)
                self.__ieskinr=Value(value=int(line[2]), lineno=lineno, index=2, file=self.fort10)
                self.__io_flag=Value(value=int(line[3]), lineno=lineno, index=3, file=self.fort10)
    ...
    @property
    def nelup(self):
        self.read(); return np.abs(self.__Nelup.v)
    @nelup.setter
    def nelup(self, value):
        self.read(); self.__Nelup.replace(value=value)
    @property
    def nel(self):
        self.read(); return np.abs(self.__Nel.v)
    @nel.setter
    def nel(self, value):
        self.read(); self.__Nel.replace(value=value)
    @property
    def natom(self):
        self.read(); return self.__Ion.v
    @natom.setter
    def natom(self, value):
        self.read(); self.__Ion.replace(value=value)
    @property
    def shell_det(self):
        self.read(); return self.__Shell_Det.v
    @shell_det.setter
    def shell_det(self, value):
        self.read(); self.__Shell_Det.replace(value=value)
    @property
    def shell_jas(self):
        self.read(); return self.__Shell_Jas.v
    @shell_jas.setter
    def shell_jas(self, value):
        self.read(); self.__Shell_Jas.replace(value=value)
    @property
    def jas_2body(self):
        self.read(); return self.__Jas_2body.v
    @jas_2body.setter
    def jas_2body(self, value):
        self.read(); self.__Jas_2body.replace(value=value)
    @property
    def det(self):
        self.read(); return self.__Det.v
    @det.setter
    def det(self, value):
        self.read(); self.__Det.replace(value=value)
    @property
    def three_body_atomic_par(self):
        self.read(); return self.__three_body_atomic_par.v
    @three_body_atomic_par.setter
    def three_body_atomic_par(self, value):
        self.read(); self.__three_body_atomic_par.replace(value=value)
    @property
    def det_mat_nonzero(self):
        self.read(); return self.__Det_mat_nonzero.v
    @det_mat_nonzero.setter
    def det_mat_nonzero(self, value):
        self.read(); self.__Det_mat_nonzero.replace(value=value)
    @property
    def jas_mat_nonzero(self):
        self.read(); return self.__Jas_mat_nonzero.v
    @jas_mat_nonzero.setter
    def jas_mat_nonzero(self, value):
        self.read(); self.__Jas_mat_nonzero.replace(value=value)
    @property
    def eq_det_atomic_par(self):
        self.read(); return self.__Eq_Det_atomic_par.v
    @eq_det_atomic_par.setter
    def eq_det_atomic_par(self, value):
        self.read(); self.__Eq_Det_atomic_par.replace(value=value)
    @property
    def eq_3_body_atomic_par(self):
        self.read(); return self.__Eq_3_body_atomic_par.v
    @eq_3_body_atomic_par.setter
    def eq_3_body_atomic_par(self, value):
        self.read(); self.__Eq_3_body_atomic_par.replace(value=value)
    @property
    def iesfree(self):
        self.read(); return self.__iesfree.v
    @iesfree.setter
    def iesfree(self, value):
        self.read(); self.__iesfree.replace(value=value)
    @property
    def iessw(self):
        self.read(); return self.__iessw.v
    @iessw.setter
    def iessw(self, value):
        self.read(); self.__iessw.replace(value=value)
    @property
    def ieskinr(self):
        self.read(); return self.__ieskinr.v
    @ieskinr.setter
    def ieskinr(self, value):
        self.read(); self.__ieskinr.replace(value=value)
    @property
    def io_flag(self):
        self.read(); return self.__io_flag.v
    @io_flag.setter
    def io_flag(self, value):
        self.read(); self.__io_flag.replace(value=value)
...
class F10structure():
    def __init__(self, fort10, Nel, Ion, start_keyword, end_keyword):
        ...
        if not self.read_flag:
            ...
            if self.__pbc:
                if not self.__tilted_flag:
                    lineno = 1
                    line = pygetline(filename=self.fort10, lineno=lineno).split()
                    self.__r_s = Value(value=float(line[0]), lineno=lineno, index=0, file=self.fort10)  # electron density parameter
                    self.__Ly_Lx = Value(value=float(line[1]), lineno=lineno, index=1, file=self.fort10)   # Ly/Lx
                    self.__Lz_Lx = Value(value=float(line[2]), lineno=lineno, index=2, file=self.fort10)   # Lz/Lx
                    self.__phase_up_1 = Value(value=float(line[3]), lineno=lineno, index=3, file=self.fort10)   # phase 1 up
                    self.__phase_up_2 = Value(value=float(line[4]), lineno=lineno, index=4, file=self.fort10)   # phase 2 up
                    self.__phase_up_3 = Value(value=float(line[5]), lineno=lineno, index=5, file=self.fort10)   # phase 3 up
                    self.__phase_dn_1 = Value(value=float(line[6]), lineno=lineno, index=6, file=self.fort10)   # phase 1 dn
                    self.__phase_dn_2 = Value(value=float(line[7]), lineno=lineno, index=7, file=self.fort10)   # phase 2 dn
                    self.__phase_dn_3 = Value(value=float(line[8]), lineno=lineno, index=8, file=self.fort10)   # phase 3 dn

                    # convert
                    # these units are bohr
                    x_length = self.__r_s.v * pow((4 * math.pi * self.nel) / (3 * self.__Ly_Lx.v * self.__Lz_Lx.v), 1.0 / 3.0)
                    ...
                    lineno=1
                    line = pygetline(filename=self.fort10, lineno=lineno).split()
                    self.__vec_a_1=Value(value=float(line[0]), lineno=lineno, index=0, file=self.fort10)
                    self.__vec_a_2=Value(value=float(line[1]), lineno=lineno, index=1, file=self.fort10)
                    self.__vec_a_3=Value(value=float(line[2]), lineno=lineno, index=2, file=self.fort10)
                    self.__vec_b_1=Value(value=float(line[3]), lineno=lineno, index=3, file=self.fort10)
                    self.__vec_b_2=Value(value=float(line[4]), lineno=lineno, index=4, file=self.fort10)
                    self.__vec_b_3=Value(value=float(line[5]), lineno=lineno, index=5, file=self.fort10)
                    self.__vec_c_1=Value(value=float(line[6]), lineno=lineno, index=6, file=self.fort10)
                    self.__vec_c_2=Value(value=float(line[7]), lineno=lineno, index=7, file=self.fort10)
                    self.__vec_c_3=Value(value=float(line[8]), lineno=lineno, index=8, file=self.fort10)
                    self.__phase_up_1 = Value(value=float(line[9]), lineno=lineno, index=9, file=self.fort10)   # phase 1 up
                    self.__phase_up_2 = Value(value=float(line[10]), lineno=lineno, index=10, file=self.fort10)   # phase 2 up
                    self.__phase_up_3 = Value(value=float(line[11]), lineno=lineno, index=11, file=self.fort10)   # phase 3 up
                    self.__phase_dn_1 = Value(value=float(line[12]), lineno=lineno, index=12, file=self.fort10)   # phase 1 dn
                    self.__phase_dn_2 = Value(value=float(line[13]), lineno=lineno, index=13, file=self.fort10)   # phase 2 dn
                    self.__phase_dn_3 = Value(value=float(line[14]), lineno=lineno, index=14, file=self.fort10)   # phase 3 dn

                    self.__vec_a = np.array([self.__vec_a_1.v, self.__vec_a_2.v, self.__vec_a_3.v])
                    self.__vec_b = np.array([self.__vec_b_1.v, self.__vec_b_2.v, self.__vec_b_3.v])
                    self.__vec_c = np.array([self.__vec_c_1.v, self.__vec_c_2.v, self.__vec_c_3.v])
            ...
            self.__valence_electrons = []
            self.__positions = []
            lines=[pygetline(filename=self.fort10, lineno=lineno, clearcache=False) for lineno in range(self.start_lineno, self.end_lineno+1)] # should be false, otherwise so slow
            ...
            for i in range(self.ion):
                p[5*i+0].type(float)
                p[5*i+1].type(float)
                p[5 * i + 2].type(float); p[5 * i + 3].type(float); p[5 * i + 4].type(float)
    ...
    def structure(self):
        ...
        __structure=Structure(cell=__cell,
    ...
    @property
    def ortho_flag(self):
        self.read(); return self.__tilted_flag
    @property
    def pbc_flag(self):
        self.read(); return self.__pbc
    @property
    def vec_a(self):
        self.read(); return self.__vec_a
    @property
    def vec_b(self):
        self.read(); return self.__vec_b
    @property
    def vec_c(self):
        self.read(); return self.__vec_c
    @property
    def norm_vec_a(self):
        self.read(); return np.linalg.norm(self.__vec_a)
    @property
    def norm_vec_b(self):
        self.read(); return np.linalg.norm(self.__vec_b)
    @property
    def norm_vec_c(self):
        self.read(); return np.linalg.norm(self.__vec_c)
    @property
    def atomic_numbers(self):
        logger.debug("atomic numbers")
        self.read(); return [i.v for i in self.__atomic_numbers]
    @property
    def valence_electrons(self):
        logger.debug("valence electrons")
        self.read(); return [i.v for i in self.__valence_electrons]
    @property
    def positions(self):
        self.read(); return np.array([[positions[0].v, positions[1].v, positions[2].v] for positions in self.__positions])
    @property
    def phase_up(self):
        self.read(); return [self.__phase_up_1.v, self.__phase_up_2.v, self.__phase_up_3.v]
    @property
    def phase_dn(self):
        self.read(); return [self.__phase_dn_1.v, self.__phase_dn_2.v, self.__phase_dn_3.v]
...
class F10forceconstraint():
    def __init__(self, fort10, ieskinr, start_keyword, end_keyword):
        self.start_keyword = start_keyword
        self.end_keyword = end_keyword
        self.fort10=fort10
        ...
        if not self.read_flag:
            ...
            for i in range(self.ieskinr):
                ...
                for num in range(np.abs(num_constraints)):
                    p[c_index + 2 * num + 1].type(int); p[c_index + 2 * num + 2].type(int)
    ...
    def constraint_num(self):
        self.read(); return [i.v for i in self.__constraint_num]
    @property
    def constraint_index(self):
        self.read(); return [i for i in self.__constraint_index]
    @property
    def atom_label(self):
        self.read(); return [i.v for i in self.__atom_label]
    @property
    def direction(self):
        self.read(); return [i.v for i in self.__direction]
    ...
    def end_lineno(self):
        return pygrep_lineno(self.fort10, self.end_keyword) - 1
class F10jastwobody():
    def __init__(self, fort10, jas_2body, start_keyword, end_keyword):
        self.start_keyword = start_keyword
        self.end_keyword = end_keyword
        self.fort10 = fort10
        self.read_flag = False
    ...
    def read(self):
        if not self.read_flag:
            ...
            lines=[pygetline(filename=self.fort10, lineno=lineno, clearcache=False) for lineno in range(self.start_lineno, self.end_lineno+1)]
            linecache.clearcache()  # a manual clearcache is better to avoid unexpected
            lineno = self.start_lineno
            p = []
            for line in lines:
                for i, value in enumerate(line.split()):
            ...
            p[0].type(int); self.__num_param.append(p[0])
            num_param=np.abs(p[0].v)
            num_twobody, flag_onebody = return_num_twobody_and_flag_onebody(jastrow_type=self.jastrow_type)
            logger.debug(self.jastrow_type)
            logger.debug(num_twobody)
            logger.debug(flag_onebody)
            if num_twobody==0:
                #why here the explicit "if" is needed?
                #because self.__num_param is not always 0! -> jastype=0 with symagp=.false.
                self.__twobody_list = []
                self.__onebody_list = []
                ...
                for i in range(num_twobody_jas):
                    p[1+i].type(float); self.__twobody_list.append(p[1+i])
                for i in range(num_onebody_jas):
                    p[1+num_twobody_jas+i].type(float); self.__onebody_list.append(p[1+num_twobody_jas+i])
    ...
    def twobody_list(self):
        self.read(); return [i.v for i in self.__twobody_list]
    @property
    def onebody_list(self):
        self.read(); return [i.v for i in self.__onebody_list]
...
class F10detbasissets():
    def __init__(self, fort10, shell_det, complex_flag, start_keyword, end_keyword):
        self.start_keyword = start_keyword
        self.end_keyword = end_keyword
        self.fort10 = fort10
        self.read_flag = False
        ...
        self.__mo_prim_index = [] # dimension (prim_ao.num, mo.num) prim_ao.num != ao.num in the turborvb implementation.
        self.__mo_coefficient = [] # dimension (prim_ao.num, mo.num) prim_ao.num != ao.num in the turborvb implementation.
        self.__mo_coefficient_imag = []  # dimension (prim_ao.num, mo.num) prim_ao.num != ao.num in the turborvb implementation.
        ...
        self.__hyb_prim_index = [] # dimension (prim_ao.num, hyb.num) prim_ao.num != ao.num in the turborvb implementation.
        self.__hyb_coefficient = [] # dimension (prim_ao.num, hyb.num) prim_ao.num != ao.num in the turborvb implementation.
        self.__hyb_coefficient_imag = []  # dimension (prim_ao.num, hyb.num) prim_ao.num != ao.num in the turborvb implementation.
    ...
    def read(self):
        if not self.read_flag:
            ...
            for num in range(abs(self.shell_det)):
                p[0].type(int); multiplicity = p[0]
                p[1].type(int); param_num = p[1]
                p[2].type(int); shell_ang_mom_turbo_notation = p[2]
                p[3].type(int); atom_label = p[3]
                ...
                if orb_type_chr == "hyb":
                    ...
                    self.__hyb_shell_ang_mom_turbo_notation.append(shell_ang_mom_turbo_notation)
                    ...
                    if not self.complex_flag: # real
                        for n in p[4:4 + num]:
                            n.type(int); hyb_prim_index.append(n)
                        for n in p[4 + num :4 + param_num.v]:
                            n.type(float); hyb_coefficient.append(n)

                    else: # complex
                        for n in p[4:4 + num]:
                            n.type(int); hyb_prim_index.append(n)
                        for i, n in enumerate(p[4 + num:4 + int(param_num.v * 3.0 / 2.0)]):
                            if i % 2 == 0:
                                n.type(float); hyb_coefficient.append(n)
                            else:
                                n.type(float); hyb_coefficient_imag.append(n)
                    ...
                    self.__mo_shell_ang_mom_turbo_notation.append(shell_ang_mom_turbo_notation)
                    ...
                    if not self.complex_flag: # real case
                        for n in p[4:4 + ind_num]:
                            n.type(int); mo_prim_index.append(n)
                        for n in p[4 + ind_num :4 + param_num.v]:
                            n.type(float); mo_coefficient.append(n)

                    else: # complex case!!
                        for n in p[4:4 + ind_num]:
                            n.type(int); mo_prim_index.append(n)
                        for i,n in enumerate(p[4 + ind_num :4 + int(param_num.v * 3.0/2.0)]):
                            if i%2 == 0:
                                n.type(float); mo_coefficient.append(n)
                            else:
                                n.type(float); mo_coefficient_imag.append(n)
                    ...
                    logger.debug(f"orb_type_num = {shell_ang_mom_turbo_notation.v}")
                    logger.debug(f"atom_label = {atom_label.v}")
                    contraction_flag = return_contraction_flag(shell_ang_mom_turbo_notation.v)
                    ...
                    if contraction_flag:
                        ...
                        p[4].type(float); self.__exponent.append(p[4])
                        self.__shell_index.append(shell_index)
    ...
    def det_basis_sets(self):
        ...
        return  Det_Basis_sets(
                    ...
                    shell_ang_mom_turbo_notation = [a.v for a in self.__shell_ang_mom_turbo_notation],
                    ...
                    hyb_shell_ang_mom_turbo_notation = [a.v for a in self.__hyb_shell_ang_mom_turbo_notation],
                    hyb_prim_index = [[a.v-1 for a in hyb_prim_list] for hyb_prim_list in self.__hyb_prim_index],
                    hyb_coefficient = [[a.v for a in hyb_coeff_list] for hyb_coeff_list in self.__hyb_coefficient],
                    hyb_coefficient_imag = [[a.v for a in hyb_coeff_imag_list] for hyb_coeff_imag_list in self.__hyb_coefficient_imag],
                )
    @property
    def shell_index(self):
        self.read(); return self.__shell_index
        #self.read(); return [[i.v for i in j] for j in self.__shell_index]
    @property
    def mo_coefficient(self):
        self.read(); return [[i.v for i in j] for j in self.__mo_coefficient]
    @property
    def mo_coefficient_imag(self):
        self.read(); return [[i.v for i in j] for j in self.__mo_coefficient_imag]
    @property
    def hyb_coefficient(self):
        self.read(); return [[i.v for i in j] for j in self.__hyb_coefficient]
    @property
    def hyb_coefficient_imag(self):
        self.read(); return [[i.v for i in j] for j in self.__hyb_coefficient_imag]
    ...
    def mo_coefficient(self, new_mo_coefficient):
        ...
        logger.debug(f"Total num sed = {len(new_mo_coefficient) * len(new_mo_coefficient[0])}")
        #self.replace_mo_coeff_with_sed(self.__mo_coefficient, new_mo_coefficient)
        self.replace_mo_coeff_pure_python(self.__mo_coefficient, new_mo_coefficient)
    @mo_coefficient_imag.setter
    def mo_coefficient_imag(self, new_mo_coefficient):
        self.read()
        logger.debug(len(new_mo_coefficient))
        logger.debug(len(self.__mo_coefficient_imag))
        logger.debug(new_mo_coefficient[0][-1])
        assert len(new_mo_coefficient) == len(self.__mo_coefficient_imag)
        logger.debug(f"Total num sed = {len(new_mo_coefficient) * len(new_mo_coefficient[0])}")
        #self.replace_mo_coeff_with_sed(self.__mo_coefficient_imag, new_mo_coefficient)
        self.replace_mo_coeff_pure_python(self.__mo_coefficient_imag, new_mo_coefficient)
    def replace_mo_coeff_with_sed(self, old_mo_coefficient, new_mo_coefficient):
        ...
        for mo_coeff, new_mo_coeff in zip(old_mo_coefficient, new_mo_coefficient):
            ...
            for coeff, new_coeff in zip(mo_coeff, new_mo_coeff):
                l=coeff.l; v=coeff.v; i=coeff.i; f=coeff.f
        ...
        file = file_list[0]
        pysed_replace_lines(file=file, lineno_list=lineno_list, value_list=value_list, index_list=index_list)
        end_sed = time.time()
    ...
    def replace_mo_coeff_pure_python(self, old_mo_coefficient, new_mo_coefficient):
        ...
        for mo_coeff, new_mo_coeff in zip(old_mo_coefficient, new_mo_coefficient):
            assert len(mo_coeff) == len(new_mo_coeff)
            line_no_list += [coeff.l for coeff in mo_coeff]
            index_list += [coeff.i for coeff in mo_coeff]
            mo_coeff_list += new_mo_coeff

        with open(self.fort10, 'r') as f:
            lines=f.readlines()

        for line_no in list(set(line_no_list)):
            line_no_index_list = [i for i, x in enumerate(line_no_list) if x == line_no]
            r_index_list=[index_list[i] for i in line_no_index_list]
            r_new_mo_coeff_list=[mo_coeff_list[i] for i in line_no_index_list]
            line=lines[line_no].split()
            for r_index, r_new_mo_coeff in zip(r_index_list, r_new_mo_coeff_list):
    ...
    def coefficient(self):
        self.read(); return [i.v for i in self.__coefficient]
    @property
    def coefficient_imag(self):
        self.read(); return [i.v for i in self.__coefficient_imag]
    @property
    def exponent(self):
        self.read(); return [i.v for i in self.__exponent]
    @property
    def shell_multiplicity(self):
        self.read(); return [i.v for i in self.__shell_multiplicity]
...
class F10jasbasissets():
    def __init__(self, fort10, shell_jas, start_keyword, end_keyword):
        self.start_keyword = start_keyword
        self.end_keyword = end_keyword
        self.fort10 = fort10
        self.read_flag = False
    ...
    def jas_basis_sets(self):
        ...
        return  Jas_Basis_sets(
                    ...
                    shell_ang_mom_turbo_notation = [a.v for a in self.__shell_ang_mom_turbo_notation[:-1]],
                    ...
                    coefficient = [a.v if a.v is not None else 1.0 for a in self.__coefficient[:-1]],
...
class F10occ():
    ...
    @property
    def occ(self):
        self.read(); return [i.v for i in self.__occupation]
    @property
    def start_lineno(self):
        return pygrep_lineno(self.fort10, self.start_keyword) + 1
    @property
    def end_lineno(self):
        return pygrep_lineno(self.fort10, self.end_keyword) - 1
class F10detmat():
    def __init__(self, fort10, detmat, complex_flag, start_keyword, end_keyword):
        self.start_keyword = start_keyword
        self.end_keyword = end_keyword
        self.fort10 = fort10
        self.read_flag = False
    ...
    def read(self):
        if not self.read_flag:
            ...
            for num in range(abs(self.detmat)):
                p[c_index+0].type(int); row = p[c_index+0]
                p[c_index+1].type(int); col = p[c_index+1]
                p[c_index+2].type(float); coeff_real = p[c_index+2]
                if self.complex_flag:
                    p[c_index + 3].type(float); coeff_imag = p[c_index + 3]
    ...
    @property
    def row(self):
        self.read(); return [i.v for i in self.__row]
    @row.setter
    def row(self, value_list):
        assert len(self.__row) == len(value_list)
        self.read()
        for row, value in zip(self.__row, value_list):
            if row.v != value: row.replace(value=value)
    @property
    def col(self):
        self.read(); return [i.v for i in self.__col]
    @col.setter
    def col(self, value_list):
        assert len(self.__col) == len(value_list)
        self.read()
        for col, value in zip(self.__col, value_list):
            if col.v != value: col.replace(value=value)
    @property
    def coeff_real(self):
        self.read(); return [i.v for i in self.__coeff_real]
    @coeff_real.setter
    def coeff_real(self, value_list):
        ...
        for coeff_real, value in zip(self.__coeff_real, value_list):
            if coeff_real.v != value: coeff_real.replace(value=value)
    @property
    def coeff_imag(self):
        self.read(); return [i.v for i in self.__coeff_imag]
    @coeff_imag.setter
    def coeff_imag(self, value_list):
        ...
        for coeff_imag, value in zip(self.__coeff_imag, value_list):
            if coeff_imag.v != value: coeff_imag.replace(value=value)
    ...
    def end_lineno(self):
        return pygrep_lineno(self.fort10, self.end_keyword) - 1
class F10jasmat():
    def __init__(self, fort10, jasmat, start_keyword, end_keyword):
        self.start_keyword = start_keyword
        self.end_keyword = end_keyword
        self.fort10 = fort10
        self.read_flag = False
...
class F10matsymmetry():
    def __init__(self, fort10, num_component, start_keyword, end_keyword):
        self.start_keyword = start_keyword
        self.end_keyword = end_keyword
        self.fort10=fort10
    ...
    def constraint_num(self, value_list):
        ...
        for constraint, value in zip(self.__constraint_num, value_list):
            if constraint.v != value: constraint.replace(value=value)
    @property
    def constraint_index(self):
        self.read(); return [i for i in self.__constraint_index]
    @property
...
class F10basissymmetry():
    ...
    def basis_index(self):
        self.read(); return [i.v for i in self.__basis_index]
...
if __name__ == "__main__":
    ...
    #print(fort10.f10detbasissets.hyb_coefficient)
