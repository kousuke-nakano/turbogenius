...
class LRDMCopt(FortranIO):
    ...
    def average_optimized_parameters(self, equil_steps=10, input_file_used="datasfnopt.input", graph_plot=False):
        ...
        if graph_plot:
            ...
            ave_story_pandas_data = pd.read_csv("./average_story.d", header=None, delim_whitespace=True,
                                                index_col=False)
        ...
        if graph_plot:
            ...
            for i in range(1, len(all_story_pandas_data.columns)):
                ...
                plt.plot(all_story_pandas_data[0].astype("int"), all_story_pandas_data[i].astype("float"),
                         color="black", marker="o", linestyle="dashed", label="all")
                plt.plot(ave_story_pandas_data[0].astype("int"), ave_story_pandas_data[i].astype("float"), color="red",
                         marker="o", linestyle="dashed", label="averaged")
                ...
                plt.gca().get_yaxis().get_major_formatter().set_useOffset(False) # No offset for y-axis
                plt.gca().get_xaxis().set_major_locator(ticker.MaxNLocator(integer=True)) # Interger for x-axis
                plt.savefig(os.path.join(graph_save_dir, "Parameter_No{}.png".format(i)), bbox_inches='tight',
                            pad_inches=0.2)
                plt.close()
        ...
        lrdmcopt = LRDMCopt.parse_from_file(file=input_file_used, in_fort10=self.in_fort10,
    ...
    def read_default_namelist():
        lrdmcopt_default_file = os.path.join(pyturbo_data_dir, "lrdmcopt", "datasfn_opt.input")
