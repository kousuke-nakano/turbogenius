...
class Cell:
    ...
    def __init__(self,
        ...
        if self.__pbc_flag==False:
            ...
            # compute cos
            self.__cos_alpha = np.inner(self.__vec_b, self.__vec_c) / (self.__norm_vec_b * self.__norm_vec_c)
            self.__cos_beta = np.inner(self.__vec_c, self.__vec_a) / (self.__norm_vec_c * self.__norm_vec_a)
            self.__cos_gamma = np.inner(self.__vec_a, self.__vec_b) / (self.__norm_vec_a * self.__norm_vec_b)

            # check if the unitcell is orthorhombic
            if self.__cos_alpha != 0.0 or self.__cos_beta != 0.0 or self.__cos_gamma != 0.0:
                logger.info("The unit cell is not orthorhombic. The tilted option is turned on.")
                self.__tilted_flag = True
            else:
                logger.info("The unit cell is orthorhombic. The tilted option is turned off.")
                self.__tilted_flag = False
    ...
    @classmethod
    def parse_cell_from_celldm(cls, celldm_1, celldm_2, celldm_3, celldm_4, celldm_5, celldm_6):
        ...
        if alpha==0.0 and beta==0.0 and gamma==0.0:
            ...
            vec_c=np.array(
                    [
                        ...
                        c * np.sqrt(1 + 2 * math.cos(alpha)*math.cos(beta)*math.cos(gamma)
...
class Structure:
    ...
    def get_ase_atom(self):
        ...
        if self.__cell.pbc_flag:
            ase_atom=Atoms(self.__element_symbols, positions=self.__positions / Angstrom)
            ase_atom.set_cell(np.array([self.__cell.vec_a / Angstrom, self.__cell.vec_b / Angstrom, self.__cell.vec_c / Angstrom]))
