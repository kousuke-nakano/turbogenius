...
from env import pyturbo_data_dir, pyturbo_root
...
class ccECP:
    ...
    def __init__(self, basis_sets_output_dir=None, pseudo_potential_output_dir=None):
    ...
    def to_file(self, element_list=None, basis_list=None):
        ...
        def add_data(p, tempdir, elements = elements):
            ...
            for r in (p.parent).glob("**/*"):
                if re.match("[A-z]{1,2}\.[A-z\-]*cc-.*\.gamess", r.name):
                    if re.match("[A-z]{1,2}\.[A-z\-]*cc-.*\.gamess", r.name):
                        basis_name = re.match("[A-z]{1,2}\.([A-z\-]*cc-.*)\.gamess", r.name).group(1)
                        ...
                        with open(r, "r") as fhandle:
                            with open(os.path.join(self.basis_sets_output_dir, f"{element}_{name}"), "w") as fhandle_out:
        ...
        for p in (tempdir/"recipes").glob("**/*"):
            if re.match("[A-z]{1,2}\.ccECP\.gamess", p.name):
                ...
                if self.ecp_output_dir is None: return
                ...
                with open(p, "r") as fhandle:
                    with open(os.path.join(self.ecp_output_dir, f"{p.name.replace('.ccECP.gamess', '_ccECP'+typ_core+'.pseudo')}"), "w") as fhandle_out:
                        fhandle_out.write(fhandle.read())
    ...
    def to_file(self, element_list, basis_list, sleep_time=1.0):
        ...
        for e, b in itertools.product(element_list, basis_list):
            ...
            try:
                ...
                E=Element(e)
                pat = re.compile("^.*?DATA.*" + E.long_name + "\n(.+)\$END", re.DOTALL | re.IGNORECASE)
                m = pat.match(basis_text)
                if (m):
                    bas=m.group(1)
                    with open(os.path.join(self.basis_sets_output_dir, f"{e}_{b}.basis"), "w") as fhandle:
                        fhandle.write(bas)
            except KeyError:
                logger.debug(f"element={e}, basis={b} do not exist in the database.")

    def all_to_file(self, sleep_time=1):
        self.to_file(element_list=chemical_symbols, basis_list=self.list_of_basis_all, sleep_time=sleep_time)
        ...
        with tqdm(titertools.product(element_list, basis_list)) as pbar:
            for i, (e,b) in enumerate(pbar):
                ...
                if not flag_downloaded:
                    ...
                    x = re.sub("\&nbsp", "", x)
                    x = re.sub(".*html.*$", "", x)
                    pat=re.compile("^.*?(" + e + "\s\d.*)("+e+".*QMC.*)$",re.M|re.DOTALL)
                    m = pat.match(x)
                    if(m):
                        bas = m.group(1)
                        ...
                        if self.ecp_output_dir is not None:
                            with open(os.path.join(self.ecp_output_dir, f"{e}_BFD.pseudo"), "w") as fo:
                                #logger.info(f"ecp -> {os.path.join(self.ecp_output_dir, f'{e}_BFD.pseudo')}")
                                fo.write(ecp)
                        if self.basis_sets_output_dir is not None:
                            if len(bas) > 15:
                                with open(os.path.join(self.basis_sets_output_dir, f"{e}_{b}.basis"), "w") as fo:
                                    #logger.info(f"basis -> {os.path.join(self.basis_sets_output_dir, f'{e}_{b}.basis')}")
                                    fo.write(bas)
                            else:
                                with open(os.path.join(self.basis_sets_output_dir, f"{e}_{b}.NaN"), "w") as fo:
                                    fo.write("NA in database")
                    else:
                        with open(os.path.join(self.ecp_output_dir, f"{e}_BFD.NaN"), "w") as fo:
                            fo.write("NA in database")
                        with open(os.path.join(self.basis_sets_output_dir, f"{e}_{b}.NaN"), "w") as fo:
                            fo.write("NA in database")
                else:
