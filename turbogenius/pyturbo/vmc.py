...
class VMC(FortranIO):
    ...
    def manual_kpoints(self, kpoints):
        ...
        self.namelist.set_parameter(parameter="double_kpgrid", value='.true.', namelist="&kpoints")
    ...
    def get_forces(self, init=10, bin=10, num_proc=1, rerun=False):
        ...
        logger.info(f"The number of forces calculated = {fort10.f10forceconstraint.ieskinr}")
        ...
        for num_force in range(fort10.f10forceconstraint.ieskinr):
            ...
            nindex_list = [i for i, x in enumerate(fort10.f10forceconstraint.constraint_index) if x == num_force]
            ...
            for nindex in nindex_list:
                ...
                force_matrix_error_bar[np.abs(atom_label) - 1, direction - 1] = force_element_error_bar
    ...
    def read_default_namelist():
        vmc_default_file = os.path.join(pyturbo_data_dir, "vmc", "datasvmc.input")
