...
from utils.utility import file_check, file_check_flag, get_line_from_file, remove_new_parameter_lines_in_fort10, remove_file
...
class VMCopt(FortranIO):
    ...
    def plot_energy_and_devmax(self, output_names=["out_min"], interactive=False):
    ...
    def get_energy(self, output_names=["out_min"]):
        ...
        energy_list = list(map(float, [i.split()[3] for i in out_min if re.match(r".*New.*Energy.*", i)]))
        error_list = list(map(float, [i.split()[4] for i in out_min if re.match(r".*New.*Energy.*", i)]))
    ...
    def get_devmax(self, output_names=["out_min"]):
        ...
        devmax_list = list(map(float, [i.split()[4] for i in out_min if re.match(r".*devmax.*par.*Normal.*", i)]))
    ...
    def get_estimated_time_for_1_generation(self, output_names=["out_min"]):
        ...
        ave_time_1000_generations = list(map(float, [i.split()[5] for i in out_min if re.match(r".*Average.*time.*for.*1000.*generations.*", i)]))
    ...
    def plot_parameters_history(self, interactive=True):
        ...
        all_story_pandas_data = pd.read_csv("./all_story.d", header=None, delim_whitespace=True, index_col=False)

        graph_save_dir=os.path.join(current_dir, "parameters_graphs")
        os.makedirs(graph_save_dir, exist_ok=True)

        if interactive:
            logger.info("How to stop to show the graphs, please type ctrl+C, then close the currently shown graph.")
        ...
        for i in range(1, len(all_story_pandas_data.columns)):
            ...
            plt.plot(all_story_pandas_data[0].astype("int"), all_story_pandas_data[i].astype("float"), color="black", marker="o", linestyle="dashed", label="all")
            ...
            if interactive:
                try:
                    ...
                    break
            plt.savefig(os.path.join(graph_save_dir, "Parameter_No{}_all.png".format(i)), bbox_inches='tight', pad_inches=0.2)
            plt.close()

    def average_optimized_parameters(self, equil_steps=10, input_file_used="datasmin.input", graph_plot=False):
        ...
        logger.info(f"The first {equil_steps} iterations are disregarded in the average.")
        ...
        ave_story_pandas_data = pd.read_csv("./average_story.d", header=None, delim_whitespace=True, index_col=False)
        ...
        if graph_plot:
            ...
            for i in range(1, len(all_story_pandas_data.columns)):
                plt.figure(figsize=(10, 8))
                plt.plot(all_story_pandas_data[0].astype("int"), all_story_pandas_data[i].astype("float"), color="black", marker="o", linestyle="dashed", label="all")
                plt.plot(ave_story_pandas_data[0].astype("int"), ave_story_pandas_data[i].astype("float"), color="red", marker="o", linestyle="dashed", label="averaged")
                plt.xlabel('Iteration', fontname="Times New Roman", fontsize=14)
                plt.ylabel('Value', fontname="Times New Roman", fontsize=14)
                # plt.legend(frameon=True)
                plt.title("Parameter_No.{}".format(i))
                ...
                plt.gca().get_yaxis().get_major_formatter().set_useOffset(False)  # No offset for y-axis
                plt.gca().get_xaxis().set_major_locator(ticker.MaxNLocator(integer=True))  # Interger for x-axis
                plt.savefig(os.path.join(graph_save_dir, "Parameter_No{}_averaged.png".format(i)), bbox_inches='tight', pad_inches=0.2)
                plt.close()
        ...
        vmcopt=VMCopt.parse_from_file(file=input_file_used, in_fort10=self.in_fort10, twist_average=self.twist_average)
        ...
        shutil.copy(os.path.join(os.getcwd(), "fort.10"), os.path.join(dir_ave_temp, "fort.10"))
        ...
        if io_fort10.pp_flag:
            ...
            shutil.copyfile(os.path.join(os.getcwd(),"pseudo.dat"), os.path.join(dir_ave_temp, "pseudo.dat"))
        ...
        shutil.move(os.path.join(current_dir, "fort.10"), os.path.join(current_dir, "fort.10_bak"))
        shutil.copyfile(os.path.join(dir_ave_temp, "fort.10"), os.path.join(current_dir, "fort.10"))
    ...
    def read_namelist_from_file(file):
        ...
        return namelist
    @classmethod
    def parse_from_default_namelist(cls, in_fort10="fort.10", twist_average=False):
